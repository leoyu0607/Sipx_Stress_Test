...
tokio::spawn(async move {
    ...
    loop {
        ...
        if let (Some(call_id), Some(code)) = (call_id, code) {
            let _ = ev_tx2.send(SipEvent::Response { call_id, code, to_tag, method, remote_rtp_port, contact_uri });
        }
    }
});
```
